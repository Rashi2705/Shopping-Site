...
Query q=s.createQuery("from Thrift_product p where p.category=:cat").setParameter("cat","homeandliving");
...
for(Thrift_product product:list){
	...
	%>
     <div class="part">
   <div class="pr"> 
   <form action="Th_Perticular_page_controller" method="post">
   <input type="hidden" name="Id" value=<%=product.getPid() %>>
   <button type="submit" href="#"  style="border:none; background-color:white">         <img src="http://localhost:8080/thriftUploads/<%= product.getTimagename()%>" alt="clothing" class ="pimg"/> 
        </button></form>
