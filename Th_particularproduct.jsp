  ...
  <%@page import="org.hibernate.Query" %>
   <%@page import="com.util.HibernetUtil" %>
    <%@page import="org.hibernate.Session" %>
     ...
     <%@ page import="com.web.Th_Perticular_page_controller"%>
...
%>
 ...
 %> 
   ...
   Integer id=(Integer)Session.getAttribute("tproid");
Session s=HibernetUtil.getFactory().openSession();
Query q=s.createQuery("from Thrift_product p where p.pid=:id").setParameter("id",id);
...
for(Thrift_product product:list){
  ...
  <section class="pro"> <div class="pr"> <a href="particularproduct.jsp">
   ...
   <input type="hidden" name="discount" value=<%=discount %> >  
     <p class="wishlist"><button class ="wish" href="wishlist.jsp">      <i class='fa fa-shopping-cart cart' style=" margin-right: 6%;font-size:20px;color: black;padding:1%;"></i>ADD TO CART</button></p></form>
 <%
	   }
    else{
    	%>
 ...
 %>
  <form action="Wishlist_controller" method="post"> <input type="hidden" name="PId" value=<%=product.getPid() %>>
   <p class="wishlist"><button class ="wish wishr" > <i  class="material-icons" style="  margin-right: 6%; font-size:20px;">favorite_border</i>WISHLIST</button></p></form>
  <%
 }
 else{
	 %>
	 <form onsubmit="wishalert()">
   <p class="wishlist"><button class ="wish wishr" href="wishlist.jsp"> <i  class="material-icons" style="  margin-right: 6%; font-size:20px;">favorite_border</i>WISHLIST</button></p></form>
  <% 
 }
  %>
   <br></br> <br></br>
